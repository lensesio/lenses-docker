...
OPTS_NEEDNOQUOTE="$OPTS_NEEDNOQUOTE LENSES_KAFKA_METRICS_PORT LENSES_KAFKA_CONNECT_CLUSTERS"
...
OPTS_SENSITIVE="$OPTS_SENSITIVE LENSES_KAFKA_SETTINGS_CONSUMER_BASIC_AUTH_USER_INFO LENSES_KAFKA_METRICS_USER LENSES_KAFKA_METRICS_PASSWORD"
...
function process_variable {
    ...
    if [[ "$OPTS_NEEDNOQUOTE" =~ " $var " ]]; then
        ...
        # Special case, these may include a password.
                     ...
                     "$var" == LENSES_SCHEMA_REGISTRY_URLS ]] && grep -sq password <<<"${!var}"; then
            echo "${conf}=********"
            unset "${var}"
        else
            echo "${conf}=${!var}"
